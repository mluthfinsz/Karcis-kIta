...
import singerImage from "../../assets/image/a7x.png"; // Pastikan path ke gambar benar
import { Link } from "react-router-dom";

const ConfirmSuccess = () => {
  const [ticketDetails] = useState({
    eventTitle: "The Life Is But A Dream",
    location: "Stadion Madya Gelora Bung Karno",
    date: "25 May 2024",
    tickets: [
      { category: "CAT 1", quantity: 2, price: 2600000 },
      { category: "CAT 2", quantity: 1, price: 1900000 },
    ],
    serviceFee: "-",
    adminFee: "-",
  });
  ...
  const calculateTotal = () => {
    ...
    return (
      ticketTotal +
      parseInt(ticketDetails.serviceFee) +
      parseInt(ticketDetails.adminFee)
    );
  };
          ...
          <Card className="p-3 border-0">
            <Card.Body>
              ...
              <div className="d-flex justify-content-between">
                ...
                <div>{parseInt(ticketDetails.serviceFee).toLocaleString()}</div>
              </div>
              <div className="d-flex justify-content-between">
                <div>Admin Fee</div>
                <div>{parseInt(ticketDetails.adminFee).toLocaleString()}</div>
              </div>
              <hr
                style={{ border: "1px dashed black ", borderStyle: "dotted" }}
              />
